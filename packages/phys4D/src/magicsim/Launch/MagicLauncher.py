...
class MagicLauncher:
    ...
    def __init__(
        ...
        self._headless: bool
        self._livestream: Literal[0, 1, 2]
        self._offscreen_render: bool
        self._sim_experience_file: str
    ...
    def add_app_launcher_args(parser: argparse.ArgumentParser) -> None:
        ...
        * ``kit_args`` (str): Optional command line arguments to be passed to Omniverse Kit directly.
        ...
        if len(config) == 0:
            ...
            MagicLauncher._check_argparser_config_params(config)
        ...
        arg_group.add_argument(
            "--headless",
            action="store_true",
            default=MagicLauncher._APPLAUNCHER_CFG_INFO["headless"][1],
            help="Force display off at all times.",
        )
        arg_group.add_argument(
            "--livestream",
            type=int,
            default=MagicLauncher._APPLAUNCHER_CFG_INFO["livestream"][1],
            choices={0, 1, 2},
        ...
        arg_group.add_argument(
            "--enable_cameras",
            action="store_true",
            default=MagicLauncher._APPLAUNCHER_CFG_INFO["enable_cameras"][1],
            help="Enable camera sensors and relevant extension dependencies.",
        )
        arg_group.add_argument(
            "--xr",
            action="store_true",
            default=MagicLauncher._APPLAUNCHER_CFG_INFO["xr"][1],
        ...
        arg_group.add_argument(
            ...
            default=MagicLauncher._APPLAUNCHER_CFG_INFO["device"][1],
    ...
    def _check_argparser_config_params(config: dict) -> None:
        ...
        applauncher_keys = set(MagicLauncher._APPLAUNCHER_CFG_INFO.keys())
        ...
        simulationapp_keys = set(MagicLauncher._SIM_APP_CFG_TYPES.keys())
        for key, value in config.items():
            if key in simulationapp_keys:
                given_type = type(value)
                expected_types = MagicLauncher._SIM_APP_CFG_TYPES[key]
    ...
    def _config_resolution(self, launcher_args: dict):
        ...
        self._sim_app_config = {
            ...
            for key in set(MagicLauncher._SIM_APP_CFG_TYPES.keys())
    ...
    def _resolve_livestream_settings(self, launcher_args: dict) -> tuple[int, int]:
        ...
        livestream_arg = launcher_args.pop(
            "livestream", MagicLauncher._APPLAUNCHER_CFG_INFO["livestream"][1]
        )
    ...
    def _resolve_headless_settings(
        ...
        headless_arg = launcher_args.pop(
            "headless", MagicLauncher._APPLAUNCHER_CFG_INFO["headless"][1]
        )
    ...
    def _resolve_camera_settings(self, launcher_args: dict):
        ...
        enable_cameras_arg = launcher_args.get(
            "enable_cameras", MagicLauncher._APPLAUNCHER_CFG_INFO["enable_cameras"][1]
        )
    ...
    def _resolve_xr_settings(self, launcher_args: dict):
        ...
        xr_arg = launcher_args.get("xr", MagicLauncher._APPLAUNCHER_CFG_INFO["xr"][1])
        xr_valid_vals = {0, 1}
    ...
    def _resolve_device_settings(self, launcher_args: dict):
        ...
        device = launcher_args.get(
            "device", MagicLauncher._APPLAUNCHER_CFG_INFO["device"][1]
        )
    ...
    def _resolve_experience_file(self, launcher_args: dict):
        ...
        magicsim_app_exp_path = os.path.dirname(os.path.abspath(__file__))
            ...
            if self._enable_cameras and not self._xr:
                if self._headless and not self._livestream:
                    self._sim_experience_file = os.path.join(
                        magicsim_app_exp_path, "magicsim.python.kit"
                    )
                else:
                    self._sim_experience_file = os.path.join(
                        magicsim_app_exp_path, "magicsim.python.kit"
                ...
                if self._headless:
                    self._sim_experience_file = os.path.join(
                        isaaclab_app_exp_path, "magicsim.python.kit"
                    )
                else:
                    self._sim_experience_file = os.path.join(
                        isaaclab_app_exp_path, "magicsim.python.kit"
    ...
    def _create_app(self):
        ...
        self._sim_app_config["enable_extensions"] = [
    ...
    def _hide_stop_button(self):
        ...
        if self._livestream >= 1 or not self._headless:
            ...
            play_button_group = toolbar._builtin_tools._play_button_group
            if play_button_group is not None:
                play_button_group._stop_button.visible = False
                play_button_group._stop_button.enabled = False
    ...
    def _set_rendering_mode_settings(self, launcher_args: dict) -> None:
        ...
        set_carb_setting(
            carb_settings, "/isaaclab/rendering/rendering_mode", rendering_mode
        )
    ...
    def _set_animation_recording_settings(self, launcher_args: dict) -> None:
        ...
        carb_settings = carb.settings.get_settings()
        set_carb_setting(
            carb_settings, "/isaaclab/anim_recording/enabled", recording_enabled
        )
        set_carb_setting(
            carb_settings, "/isaaclab/anim_recording/start_time", start_time
        )
    ...
    def _hide_play_button(self, flag):
        ...
        if self._livestream >= 1 or not self._headless:
            ...
            play_button_group = toolbar._builtin_tools._play_button_group
            if play_button_group is not None:
                play_button_group._play_button.visible = not flag
                play_button_group._play_button.enabled = not flag
