...
class Settings:
    ...
    def extend_data_generation_length(cls):
        return CarbSettingUtil.get_value_by_key(
            key=cls.EXTEND_DATA_GENERATION_LENGTH,
            fallback_value=0,
...
class AssetPaths:
    ...
    async def cache_paths_async(cls):
        try:
            ...
            carb.log_info(
                f"Cache Isaac Sim assets root path: {cls.cached_asset_default_root_path}."
            )
        except RuntimeError:
            carb.log_warn(
                "Finding Isaac Sim assets root path has encountered runtime error. Will use fallback asset paths instead."
            )
        except asyncio.CancelledError:
            carb.log_warn(
                "Finding Isaac Sim assets root path has been cancelled unexpectedly. Will use fallback asset paths instead."
            )
        except TimeoutError:
            carb.log_warn(
                "Timeout for finding Isaac Sim assets root path. Will use fallback asset paths instead."
            )

    @classmethod
    def exclusive_character_folders(cls):
        return CarbSettingUtil.get_value_by_key(
            key=cls.EXCLUSIVE_CHARACTER_FOLDERS,
            fallback_value=["biped_demo"],
            override_setting=True,
        )
    ...
    def _get_asset_carb_value(
        cls, carb_setting_key, default_path_from_root, fallback_path
    ):
...
class BehaviorScriptPaths:
    ...
    def behavior_script_path(cls):
        ...
        return CarbSettingUtil.get_value_by_key(
            key=cls.DEFAULT_BEHAVIOR_SCRIPT_PATH,
            fallback_value=fallback_path,
            override_setting=True,
        )
...
class PrimPaths:
    ...
    def characters_parent_path(cls):
        return CarbSettingUtil.get_value_by_key(
            key=cls.CHARACTERS_PARENT_PATH,
            fallback_value="/World/Characters",
            override_setting=True,
        )
    ...
    def robots_parent_path(cls):
        return CarbSettingUtil.get_value_by_key(
            key=cls.ROBOTS_PARENT_PATH,
            fallback_value="/World/Robots",
            override_setting=True,
        )
    ...
    def cameras_parent_path(cls):
        return CarbSettingUtil.get_value_by_key(
            key=cls.CAMERAS_PARENT_PATH,
            fallback_value="/World/Cameras",
            override_setting=True,
        )
    ...
    def lidar_cameras_parent_path(cls):
        return CarbSettingUtil.get_value_by_key(
            key=cls.LIDAR_CAMERAS_PARENT_PATH,
            fallback_value="/World/Lidars",
            override_setting=True,
        )
...
class CommandSetting:
    ...
    def set_character_goto_min_distance(cls, value: float):
        return CarbSettingUtil.set_value_by_key(
    ...
    def get_character_interact_object_root_path(cls):
        return CarbSettingUtil.get_value_by_key(
            key=cls.CHARACTER_INTERACT_OBJECT_ROOT_PATH
        )
