...
def make_plane(
    size: tuple[float, float], height: float, center_zero: bool = True
) -> trimesh.Trimesh:
...
def make_border(
    ...
    box_mesh_top = trimesh.creation.box(
    ...
    box_mesh_bottom = trimesh.creation.box(
        box_dims, trimesh.transformations.translation_matrix(box_pos)
    )
    ...
    box_mesh_left = trimesh.creation.box(
        box_dims, trimesh.transformations.translation_matrix(box_pos)
    )
    ...
    box_mesh_right = trimesh.creation.box(
        box_dims, trimesh.transformations.translation_matrix(box_pos)
    )
...
def make_cylinder(
    ...
    return trimesh.creation.cylinder(
        radius, height, sections=np.random.randint(4, 6), transform=transform
    )
