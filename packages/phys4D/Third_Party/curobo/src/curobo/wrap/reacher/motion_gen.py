...
class MotionGenResult:
    ...
    def get_paths(self) -> List[JointState]:
        ...
        if isinstance(self.interpolated_plan, list):
            num_plans = len(self.interpolated_plan)
            if self.path_buffer_last_tstep is None:
                return self.interpolated_plan

            if len(self.path_buffer_last_tstep) < num_plans:

                if len(self.path_buffer_last_tstep) > 0:
                    last_val = self.path_buffer_last_tstep[-1]
                    self.path_buffer_last_tstep = self.path_buffer_last_tstep + [last_val] * (num_plans - len(self.path_buffer_last_tstep))
                else:

                    return self.interpolated_plan
            path = [
                self.interpolated_plan[x].trim_trajectory(0, self.path_buffer_last_tstep[x])
                for x in range(num_plans)
            ]
            return path



        if self.interpolated_plan.position.ndim == 3:

            num_plans = self.interpolated_plan.position.shape[0]
        elif self.interpolated_plan.position.ndim == 2:
        ...
        if self.path_buffer_last_tstep is None:
            ...
            if num_plans == 1:
                ...
                if self.interpolated_plan.position.ndim == 3 and self.interpolated_plan.position.shape[0] == 1:
                    single_js = JointState(
                        position=self.interpolated_plan.position.squeeze(0),
                        velocity=self.interpolated_plan.velocity.squeeze(0) if self.interpolated_plan.velocity is not None else None,
                        acceleration=self.interpolated_plan.acceleration.squeeze(0) if self.interpolated_plan.acceleration is not None else None,
                        jerk=self.interpolated_plan.jerk.squeeze(0) if self.interpolated_plan.jerk is not None else None,
                        joint_names=self.interpolated_plan.joint_names,
                    )
                    return [single_js]
                else:
                    return [self.interpolated_plan]
                ...
                for i in range(num_plans):
                    ...
                    single_js = JointState(
                        position=self.interpolated_plan.position[i].squeeze(0) if self.interpolated_plan.position.ndim == 3 else self.interpolated_plan.position[i],
                        velocity=self.interpolated_plan.velocity[i].squeeze(0) if self.interpolated_plan.velocity is not None and self.interpolated_plan.velocity.ndim == 3 else (self.interpolated_plan.velocity[i] if self.interpolated_plan.velocity is not None else None),
                        acceleration=self.interpolated_plan.acceleration[i].squeeze(0) if self.interpolated_plan.acceleration is not None and self.interpolated_plan.acceleration.ndim == 3 else (self.interpolated_plan.acceleration[i] if self.interpolated_plan.acceleration is not None else None),
                        jerk=self.interpolated_plan.jerk[i].squeeze(0) if self.interpolated_plan.jerk is not None and self.interpolated_plan.jerk.ndim == 3 else (self.interpolated_plan.jerk[i] if self.interpolated_plan.jerk is not None else None),
                        joint_names=self.interpolated_plan.joint_names,
                    )
                    path_list.append(single_js)
                return path_list
        ...
        path = []
        for x in range(num_plans):
            if x < len(self.path_buffer_last_tstep) and self.path_buffer_last_tstep[x] is not None:
                last_tstep = self.path_buffer_last_tstep[x]
                if num_plans == 1:

                    trimmed_js = self.interpolated_plan.trim_trajectory(0, last_tstep)

                    if trimmed_js.position.ndim == 3 and trimmed_js.position.shape[0] == 1:
                        trimmed_js = JointState(
                            position=trimmed_js.position.squeeze(0),
                            velocity=trimmed_js.velocity.squeeze(0) if trimmed_js.velocity is not None else None,
                            acceleration=trimmed_js.acceleration.squeeze(0) if trimmed_js.acceleration is not None else None,
                            jerk=trimmed_js.jerk.squeeze(0) if trimmed_js.jerk is not None else None,
                            joint_names=trimmed_js.joint_names,
                        )
                    path.append(trimmed_js)
                else:


                    single_js = JointState(
                        position=self.interpolated_plan.position[x] if self.interpolated_plan.position.ndim == 3 else self.interpolated_plan.position[x : x + 1],
                        velocity=self.interpolated_plan.velocity[x] if self.interpolated_plan.velocity is not None and self.interpolated_plan.velocity.ndim == 3 else (self.interpolated_plan.velocity[x : x + 1] if self.interpolated_plan.velocity is not None else None),
                        acceleration=self.interpolated_plan.acceleration[x] if self.interpolated_plan.acceleration is not None and self.interpolated_plan.acceleration.ndim == 3 else (self.interpolated_plan.acceleration[x : x + 1] if self.interpolated_plan.acceleration is not None else None),
                        jerk=self.interpolated_plan.jerk[x] if self.interpolated_plan.jerk is not None and self.interpolated_plan.jerk.ndim == 3 else (self.interpolated_plan.jerk[x : x + 1] if self.interpolated_plan.jerk is not None else None),
                        joint_names=self.interpolated_plan.joint_names,
                    )
                    trimmed_js = single_js.trim_trajectory(0, last_tstep)
                    path.append(trimmed_js)
            else:
                ...
                if num_plans == 1:
                    ...
                    if self.interpolated_plan.position.ndim == 3 and self.interpolated_plan.position.shape[0] == 1:
                        ...
                        path.append(single_js)
                    else:
                        path.append(self.interpolated_plan)
                else:
        ...
        if source_tensor is not None:
            if current_tensor is None:
                ...
                if isinstance(current_tensor, torch.Tensor) and isinstance(
                    ...
                    if current_tensor.dim() == 0 and source_tensor.dim() == 0:

                        current_tensor.copy_(source_tensor)
                    elif current_tensor.dim() == 0:


                        if source_tensor.dim() > 0 and idx < source_tensor.shape[0]:
                            current_tensor.copy_(source_tensor[idx] if source_tensor.dim() > 0 else source_tensor)
                        else:
                            current_tensor.copy_(source_tensor)
                    elif source_tensor.dim() == 0:


                        if current_tensor.dim() > 0 and idx < current_tensor.shape[0]:
                            current_tensor[idx] = source_tensor
                        else:
                            current_tensor.copy_(source_tensor.unsqueeze(0) if current_tensor.dim() > 0 else source_tensor)
                    else:
...
class MotionGen(MotionGenConfig):
    ...
    def plan_batch_env(
        ...
        Args:
            ...
            env_query_idx: Optional tensor of shape [batch_size] or [batch_size, 1] specifying
                which env_id each batch element should map to. If None, maps batch index i to env_id i.
                This allows selective planning for specific environments (e.g., only plan for env 0 and 2).
        ...
        solve_state = self._get_solve_state(
            ReacherSolveType.BATCH_ENV, plan_config, goal_pose, start_state, env_query_idx=env_query_idx
        )
        result = self._plan_batch_attempts(
            solve_state,
            start_state,
    ...
    def plan_batch_env_goalset(
        ...
        Args:
            ...
            link_poses: Goal poses for each link in the robot when planning for multiple links.
            env_query_idx: Optional tensor of shape [batch_size] or [batch_size, 1] specifying
                which env_id each batch element should map to. If None, batch index i maps to env_id i.
        ...
        if plan_config.enable_graph_attempt is not None:
            ...
            plan_config.enable_graph_attempt = None
        solve_state = self._get_solve_state(
            ReacherSolveType.BATCH_ENV_GOALSET, plan_config, goal_pose, start_state, env_query_idx=env_query_idx
        )
        result = self._plan_batch_attempts(
            solve_state,
            start_state,
    ...
    def _solve_ik_from_solve_state(
        ...
        if solve_state.batch_env and env_query_idx is not None:
            self.ik_solver._env_query_idx = env_query_idx
        else:
            self.ik_solver._env_query_idx = None
        ...
        if plan_config.enable_opt:
            with profiler.record_function("motion_gen/setup_trajopt_seeds"):
                ...
                if (
                    ...
                    goal_config = self._trajopt_goal_config[0]
    ...
    def _plan_from_solve_state_batch(
        ...
        if plan_config.enable_opt:
            ...
            goal_config = self._trajopt_goal_config


            if solve_state.batch_env and env_query_idx is not None:

                goal = solve_state.create_goal_buffer(
                    goal_pose,
                    None,
                    None,
                    link_poses,
                ...
                goal.current_state = start_state
            else:
                goal = Goal(
                    goal_pose=goal_pose,
                    current_state=start_state,
            ...
            if trajopt_seed_traj is None or (
                ...
                if solve_state.batch_env and env_query_idx is not None:
                    ...
                    repeated_env_query_idx = env_query_idx.repeat_interleave(solve_state.num_trajopt_seeds)


                    goal_config_reshaped = goal_config.view(-1, self._dof)

                    repeated_batch_size = solve_state.batch_size * solve_state.num_trajopt_seeds
                    seed_goal = Goal.create_idx(
                        pose_batch_size=repeated_batch_size,
                        batch_env=solve_state.batch_env,
                        batch_retract=False,
                        num_seeds=1,
                        tensor_args=self.tensor_args,
                        env_query_idx=repeated_env_query_idx,
                    )
                    seed_goal.goal_pose = goal_pose.repeat_seeds(solve_state.num_trajopt_seeds)
                    seed_goal.current_state = start_state.repeat_seeds(solve_state.num_trajopt_seeds)
                    seed_goal.goal_state = JointState.from_position(goal_config_reshaped)
                    if seed_link_poses is not None:
                        seed_goal.links_goal_pose = seed_link_poses

                    seed_goal._update_batch_size()

                    if seed_goal.batch != repeated_batch_size:
                        seed_goal.batch = repeated_batch_size
                else:
                    seed_goal = Goal(
                        goal_pose=goal_pose.repeat_seeds(solve_state.num_trajopt_seeds),
                        current_state=start_state.repeat_seeds(solve_state.num_trajopt_seeds),
