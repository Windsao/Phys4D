...
class TrajOptSolver(TrajOptSolverConfig):
    ...
    def _solve_from_solve_state(
        ...
        goal_buffer = self._update_solve_state_and_goal_buffer(solve_state, goal, env_query_idx=env_query_idx)
    ...
    def solve_batch_env(
        ...
        if goal.batch <= 0:
            ...
            if goal.goal_pose is not None and goal.goal_pose.batch > 0:
                ...
                raise ValueError(f"Cannot determine goal.batch: goal.batch={goal.batch}, goal_pose.batch={goal.goal_pose.batch if goal.goal_pose is not None else None}")




        if env_query_idx is not None and env_query_idx.shape[0] == goal.batch:

            inferred = False
            if seed_traj is not None and len(seed_traj.shape) >= 4:
                actual_total_seeds = seed_traj.shape[0] * seed_traj.shape[1]
                if actual_total_seeds % goal.batch == 0:
                    inferred_num_seeds = actual_total_seeds // goal.batch
                    if inferred_num_seeds > 0:
                        num_seeds = inferred_num_seeds
                        inferred = True
            if not inferred:
                num_seeds = 1
        solve_state = ReacherSolveState(
    ...
    def get_seed_set(
        self,
        goal: Goal,
        seed_traj: Union[JointState, torch.Tensor, None] = None,
        seed_success: Optional[torch.Tensor] = None,
        num_seeds: Optional[int] = None,
        batch_mode: bool = False,
    ):
        ...
        if seed_traj is None:
            ...
            lin_seed_traj[seed_success] = seed_traj
        ...
        actual_total_seeds = None
        if len(seed_traj.shape) == 3:
            actual_total_seeds = seed_traj.shape[0]
            if seed_traj.shape[1] != self.action_horizon or seed_traj.shape[2] != self.dof:
                log_error(
            ...
            actual_total_seeds = seed_traj.shape[0] * seed_traj.shape[1]
            if seed_traj.shape[2] != self.action_horizon or seed_traj.shape[3] != self.dof:
                log_error(
                    "Seed traj shape should be [num_seeds, batch, action_horizon, dof]"
        ...
        if actual_total_seeds is None:
            actual_total_seeds = total_seeds

        if actual_total_seeds != total_seeds:
            reconciled = False
            if goal.batch > 0 and actual_total_seeds % goal.batch == 0:
                inferred_num_seeds = actual_total_seeds // goal.batch
                if inferred_num_seeds > 0:
                    num_seeds = inferred_num_seeds
                    total_seeds = actual_total_seeds
                    reconciled = True
            if not reconciled and num_seeds > 0 and actual_total_seeds % num_seeds == 0:
                inferred_batch = actual_total_seeds // num_seeds
                goal.batch = inferred_batch
                total_seeds = actual_total_seeds
                reconciled = True
            if not reconciled:
                log_error(
                    "Unable to reconcile seed traj shape with expected total seeds. "
                    + f"Expected total_seeds={total_seeds}, got shape {seed_traj.shape} "
                    + f"(actual_total={actual_total_seeds}, goal.batch={goal.batch}, "
                    + f"num_seeds={num_seeds})"
                )

        seed_traj = seed_traj.reshape(actual_total_seeds, self.action_horizon, self.dof)
        return seed_traj
