...
def smooth_cost(abs_acc, abs_jerk, opt_dt) -> torch.Tensor:
    ...
    mean_acc = torch.mean(torch.max(abs_acc, dim=-1)[0], dim=-1)
...
def compute_smoothness(
    ...
    max_v_arr = torch.max(torch.abs(vel), dim=-2)[0]
    scale_dt = (max_v_arr) / (max_vel.view(1, max_v_arr.shape[-1]))

    max_acc_arr = torch.max(torch.abs(acc), dim=-2)[0]
    max_acc = max_acc.view(1, max_acc_arr.shape[-1])
    scale_dt_acc = torch.sqrt(torch.max(max_acc_arr / max_acc, dim=-1)[0])

    max_jerk_arr = torch.max(torch.abs(jerk), dim=-2)[0]

    max_jerk = max_jerk.view(1, max_jerk_arr.shape[-1])
    scale_dt_jerk = torch.pow(torch.max(max_jerk_arr / max_jerk, dim=-1)[0], 1.0 / 3.0)

    dt_score_vel = torch.max(scale_dt, dim=-1)[0]
    ...
    max_acc_val = torch.max(abs_acc, dim=-2)[0]
    abs_jerk = torch.abs(jerk) * scale_dt**3



    max_jerk_val = torch.max(abs_jerk, dim=-2)[0]
