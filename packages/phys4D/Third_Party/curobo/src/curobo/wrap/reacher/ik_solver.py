...
class IKSolver(IKSolverConfig):
    ...
    def solve_batch_env(
        ...
        env_query_idx_for_state = env_query_idx
        if env_query_idx is not None:

            if len(env_query_idx.shape) > 1:
                env_query_idx_for_state = env_query_idx.view(-1)
            else:
                env_query_idx_for_state = env_query_idx
            n_envs = int(env_query_idx_for_state.max().item() + 1) if len(env_query_idx_for_state) > 0 else goal_pose.batch
        else:
        ...
        solve_state = ReacherSolveState(
            ...
            n_envs=n_envs,
            n_goalset=1,
            env_query_idx=env_query_idx_for_state,
        )
        return self._solve_from_solve_state(
            solve_state,
            goal_pose,
    ...
    def _solve_from_solve_state(
        ...
        if solve_state.env_query_idx is not None:
            env_query_idx = solve_state.env_query_idx
        elif env_query_idx is None and hasattr(self, '_env_query_idx') and self._env_query_idx is not None:
            env_query_idx = self._env_query_idx
        goal_buffer = self._update_goal_buffer(solve_state, goal_pose, retract_config, link_poses, env_query_idx=env_query_idx)
