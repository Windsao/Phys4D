...
class WrapBase(WrapConfig):
    ...
    def solve(self, goal: Goal, seed: Optional[torch.Tensor] = None):
        ...
        if goal.current_state is None:
            if goal.goal_state is not None:
                goal.current_state = goal.goal_state.clone()
            elif hasattr(self.safety_rollout, "init_state") and self.safety_rollout.init_state is not None:
                goal.current_state = self.safety_rollout.init_state.clone()
            else:
                raise ValueError(
                    "Goal current_state is None and no fallback state is available to initialize it."
                )
