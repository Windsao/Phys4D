...
class Goal(Sequence):
    ...
    batch_pose_idx: Optional[torch.Tensor] = None
    batch_goal_state_idx: Optional[torch.Tensor] = None
    batch_retract_state_idx: Optional[torch.Tensor] = None
    batch_current_state_idx: Optional[torch.Tensor] = None
    batch_enable_idx: Optional[torch.Tensor] = None
    batch_world_idx: Optional[torch.Tensor] = None
    ...
    def create_index_buffers(
        ...
        new_goal = Goal.create_idx(batch_size, batch_env, batch_retract, num_seeds, tensor_args, env_query_idx=env_query_idx)
    ...
    def create_idx(
        ...
        if batch_env:
            if env_query_idx is not None:
                ...
                if env_query_idx.shape[0] != pose_batch_size:
                    log_error(
                        f"env_query_idx shape {env_query_idx.shape} does not match pose_batch_size {pose_batch_size}"
                    )
                batch_world_idx = env_query_idx.to(
                    device=tensor_args.device, dtype=torch.int32
                ).unsqueeze(-1) if len(env_query_idx.shape) == 1 else env_query_idx.to(
                    device=tensor_args.device, dtype=torch.int32
                )
            else:
