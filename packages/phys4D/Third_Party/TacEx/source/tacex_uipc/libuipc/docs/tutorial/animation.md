...
=== "C++"
    ...
    ```cpp
    RotatingMotor rm; rm.name();
    LinearMotor lm; lm.name();
    ```
...
`SoftTransformConstraint` is a general constraint that can be used to fully control the transformation of a geometry, which may require some mathematical knowledge of [transformation matrix](https:
...
=== "Python"
    ...
    source: [periodically_pressed_tetrahedron](https: