...
ParallelFor(256)
    ...
    .apply(N,
           [dC = dC.viewer().name("dC"),
            dA = dA.cviewer().name("dA"),
            dB = dB.cviewer().name("dB")]
           __device__(int i) mutable
           {
               dC(i + 1) = dA(i) + dB(i);
           })
...
BufferLaunch(s)
    .copy(d_buffer.view(0,5), h_buffer.data())
    .wait();
```
...
The generated graphviz code can be viewed on [Graphviz Online](https:
