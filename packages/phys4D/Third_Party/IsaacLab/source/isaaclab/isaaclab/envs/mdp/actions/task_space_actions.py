...
class OperationalSpaceControllerAction(ActionTerm):
    ...
    def __init__(self, cfg: actions_cfg.OperationalSpaceControllerActionCfg, env: ManagerBasedEnv):
        ...
        self._ee_pose_b_no_offset = torch.zeros(self.num_envs, 7, device=self.device)
        ...
        self._ee_force_w = torch.zeros(self.num_envs, 3, device=self.device)
        self._ee_force_b = torch.zeros(self.num_envs, 3, device=self.device)
    ...
    def _compute_ee_jacobian(self):
        ...
        if self.cfg.body_offset is not None:
            ...
            self._jacobian_b[:, 0:3, :] += torch.bmm(-math_utils.skew_symmetric_matrix(self._offset_pos), self._jacobian_b[:, 3:, :])
    ...
    def _compute_ee_force(self):
        ...
        if self._contact_sensor is not None:
            ...
            self._ee_force_w[:] = self._contact_sensor.data.net_forces_w[:, 0, :]
