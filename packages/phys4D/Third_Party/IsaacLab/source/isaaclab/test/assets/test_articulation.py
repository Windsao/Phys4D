...
from isaaclab_assets import ANYMAL_C_CFG, FRANKA_PANDA_CFG, SHADOW_HAND_CFG
...
def test_joint_effort_limits(sim, num_articulations, device, add_ground_plane):
    ...
    out = joint_effort_out_of_limit(env, robot_all)
    assert torch.all(~out)


    articulation._data.computed_torque.fill_(100.0)
    articulation._data.applied_torque.fill_(50.0)
    out = joint_effort_out_of_limit(env, robot_all)
    assert torch.all(out)
...
def test_setting_effort_limit_explicit(sim, num_articulations, device, effort_limit_sim, effort_limit):
    ...
    if effort_limit_sim is not None:
        ...
        limit = ActuatorBase._DEFAULT_MAX_EFFORT_SIM
