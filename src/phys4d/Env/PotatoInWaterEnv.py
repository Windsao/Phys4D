...
class PotatoInWaterEnv(TaskCameraBaseEnv):
    """
    Potato In Water environment for automatic data collection.
    ...
    def __init__(self, config, cli_args, logger):
        ...
        self._prev_potato_positions: Dict[int, torch.Tensor] = {}
    ...
    def reset(self, seed: int | None = None, options: dict[str, Any] | None = None):
        """Reset all environments and clear previous potato positions."""
        obs, info = super().reset(seed=seed, options=options)

        self._prev_potato_positions.clear()
        return obs, info
    ...
    def reset_idx(
        ...
        """Reset specific environments and clear their previous potato positions."""
        ...
        if env_ids is not None:
            ...
            for env_id in env_ids:
                env_id_int = int(env_id)
                if env_id_int in self._prev_potato_positions:
                    del self._prev_potato_positions[env_id_int]
        else:
            self._prev_potato_positions.clear()
        return obs, info
    ...
    def get_done(
        ...
        """
        Check if potato_1 has come to rest by comparing position changes between steps.
        ...
        Returns:
            Tensor of shape (num_envs,) with True for environments where potato is rested, False otherwise.
        """
        if env_ids is None:
            env_ids = torch.arange(
                self.scene.num_envs, device=self.device, dtype=torch.long
        ...
        for env_id in env_ids:
            ...
            if (
                "potato" in self.scene.scene_manager._rigid_objects[env_id_int]
                and len(self.scene.scene_manager._rigid_objects[env_id_int]["potato"])
                > 0
            ):
                potato = self.scene.scene_manager._rigid_objects[env_id_int]["potato"][
                    0
                ]
                potato_found = True

            elif (
                "potato" in self.scene.scene_manager._deformable_objects[env_id_int]
                and len(
                    self.scene.scene_manager._deformable_objects[env_id_int]["potato"]
                )
                > 0
            ):
                potato = self.scene.scene_manager._deformable_objects[env_id_int][
                    "potato"
                ][0]
                potato_found = True

            if not potato_found or potato is None:
                is_done[env_id_int] = False
                if debug and env_id_int == 0:
                    print("  env_0: Potato not found in rigid or deformable objects")
                continue

            try:
                potato_state = potato.get_state(is_relative=False)

                if "root_pose" not in potato_state:
                    is_done[env_id_int] = False
                    if debug and env_id_int == 0:
                        print("  env_0: root_pose not in state")
                    continue

                root_pose = potato_state["root_pose"]
                ...
                if debug and env_id_int == 0:
                    ...
                    if env_id_int in self._prev_potato_positions:
                        prev_pos = self._prev_potato_positions[env_id_int]
                ...
                self._prev_potato_positions[env_id_int] = current_pos.clone()
