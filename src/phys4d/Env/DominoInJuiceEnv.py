...
class DominoInJuiceEnv(TaskCameraBaseEnv):
    """
    phys4d: Domino In Juice scene.
    get_done  domino_1
    """
    ...
    def __init__(self, config, cli_args, logger):
        ...
        self._prev_domino_positions: Dict[int, torch.Tensor] = {}
    ...
    def reset(self, seed: int | None = None, options: dict[str, Any] | None = None):
        """Reset all environments and clear previous domino positions."""
        obs, info = super().reset(seed=seed, options=options)

        self._prev_domino_positions.clear()
        return obs, info
    ...
    def reset_idx(
        ...
        """Reset specific environments and clear their previous domino positions."""
        ...
        if env_ids is not None:
            ...
            for env_id in env_ids:
                env_id_int = int(env_id)
                if env_id_int in self._prev_domino_positions:
                    del self._prev_domino_positions[env_id_int]
        else:
            self._prev_domino_positions.clear()
        return obs, info
    ...
    def get_done(
        ...
        Returns:
            Tensor of shape (num_envs,) with True for environments where domino_1 is rested, False otherwise.
        """
        if env_ids is None:
            env_ids = torch.arange(
                self.scene.num_envs, device=self.device, dtype=torch.long
        ...
        for env_id in env_ids:
            ...
            if (
                "domino" in self.scene.scene_manager._rigid_objects[env_id_int]
                and len(self.scene.scene_manager._rigid_objects[env_id_int]["domino"])
                > 0
            ):
                domino_1 = self.scene.scene_manager._rigid_objects[env_id_int][
                    "domino"
                ][0]

                try:
                    domino_state = domino_1.get_state(is_relative=False)

                    if "root_pose" not in domino_state:
                        is_done[env_id_int] = False
                        if debug and env_id_int == 0:
                            print("  env_0: root_pose not in state")
                        continue

                    root_pose = domino_state["root_pose"]
                    ...
                    if debug and env_id_int == 0:
                        ...
                        if env_id_int in self._prev_domino_positions:
                            prev_pos = self._prev_domino_positions[env_id_int]
                    ...
                    self._prev_domino_positions[env_id_int] = current_pos.clone()
                ...
                is_done[env_id_int] = False
                if debug and env_id_int == 0:
                    print("  env_0: domino_1 not found in rigid_objects")
