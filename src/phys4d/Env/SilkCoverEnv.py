...
class SilkCoverEnv(TaskCameraBaseEnv):
    """
    Silk Cover environment for automatic data collection.
    This environment is designed for collecting data from phys4d scenes.
    get_done silk_cloth_1
    """

    def __init__(self, config, cli_args, logger):
        super().__init__(config, cli_args, logger)
        self._primary_camera_name: str | None = None
        self._prev_cloth_positions: Dict[int, torch.Tensor] = {}
    ...
    def reset(self, seed: int | None = None, options: dict[str, Any] | None = None):
        """Reset all environments and clear previous cloth positions."""
        obs, info = super().reset(seed=seed, options=options)

        self._prev_cloth_positions.clear()
        return obs, info
    ...
    def reset_idx(
        ...
        """Reset specified environments and clear their previous cloth positions."""
        ...
        if env_ids is not None:
            ...
            for env_id in env_ids:
                env_id_int = int(env_id)
                if env_id_int in self._prev_cloth_positions:
                    del self._prev_cloth_positions[env_id_int]
        else:
            self._prev_cloth_positions.clear()
    ...
    def get_done(
        ...
        """
        Check if silk_cloth_1 has come to rest by comparing position changes between steps.
        """
        if env_ids is None:
            env_ids = torch.arange(
                self.scene.num_envs, device=self.device, dtype=torch.long
        ...
        for env_id in env_ids:
            ...
            if (
                "silk_cloth" in self.scene.scene_manager._garment_objects[env_id_int]
                and len(
                    self.scene.scene_manager._garment_objects[env_id_int]["silk_cloth"]
                )
                > 0
            ):
                cloth = self.scene.scene_manager._garment_objects[env_id_int][
                    "silk_cloth"
                ][0]
                cloth_found = True

            elif (
                "silk_cloth" in self.scene.scene_manager._rigid_objects[env_id_int]
                and len(
                    self.scene.scene_manager._rigid_objects[env_id_int]["silk_cloth"]
                )
                > 0
            ):
                cloth = self.scene.scene_manager._rigid_objects[env_id_int][
                    "silk_cloth"
                ][0]
                cloth_found = True
            ...
            if not cloth_found:
                ...
                if debug and env_id_int == 0:
                    print(
                        "  env_0: Silk cloth not found in deformable or rigid objects"
                    )
                continue

            try:
                cloth_state = cloth.get_state(is_relative=False)

                if "root_pose" not in cloth_state:
                    is_done[env_id_int] = False
                    if debug and env_id_int == 0:
                        print("  env_0: root_pose not in state for silk_cloth")
                    continue

                root_pose = cloth_state["root_pose"]
                ...
                if torch.isnan(current_pos).any():
                    is_done[env_id_int] = False
                    if debug and env_id_int == 0:
                        print("  env_0: silk_cloth root_pose contains nan")
                    continue

                if debug and env_id_int == 0:
                    pos_str = f"[{current_pos[0]:.4f}, {current_pos[1]:.4f}, {current_pos[2]:.4f}]"
                    if env_id_int in self._prev_cloth_positions:
                        prev_pos = self._prev_cloth_positions[env_id_int]
                        pos_change = torch.norm(current_pos - prev_pos).item()
                        print(
                            f"Step: env_0 silk_cloth pos={pos_str}, pos_change={pos_change:.6f}, threshold={position_threshold} (from get_state)"
                        )
                    else:
                        print(
                            f"Step: env_0 silk_cloth pos={pos_str} (first step, no previous pos, from get_state)"
                        )
                ...
                if env_id_int in self._prev_cloth_positions:
                    ...
                    if debug and env_id_int == 0:
                        print(
                            f"  env_0: silk_cloth is_done={is_done[env_id_int].item()}, pos_change={pos_change.item():.6f}"
                        )
                else:
                    is_done[env_id_int] = False

                self._prev_cloth_positions[env_id_int] = current_pos.clone()
            except Exception as e:
                is_done[env_id_int] = False
                if debug and env_id_int == 0:
                    print(f"  env_0: Error getting silk_cloth position: {e}")
                    import traceback
