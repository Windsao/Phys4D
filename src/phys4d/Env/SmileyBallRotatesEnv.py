...
class SmileyBallRotatesEnv(TaskCameraBaseEnv):
    """
    Smiley Ball Rotates  cylinder + ball_on_cylinderget_done  step>=80

    -  TaskCameraBaseEnv
    -  _init_cylinder_controllers BallSpinController  rotating_cylinder + ball_on_cylinder
    - get_done sim.step  80  True
    """
    ...
    def __init__(self, config, cli_args, logger):
        ...
        self._cylinder_controllers: Dict[int, "_BallSpinController"] = {}
    ...
    def _init_cylinder_controllers(self):
        """env rotating_cylinder + ball_on_cylinder"""
        angular_speed = 4.0
        ...
        for env_id in range(self.num_envs):
            ...
            objects_ball = self.scene.scene_manager.get_objects(
                env_ids=[env_id], object_name="ball_on_cylinder", object_type="rigid"
            )
            ball = None
            for obj_list in objects_ball.values():
                if obj_list:
                    ball = obj_list[0]
                    break
            ...
            self._cylinder_controllers[env_id] = _BallSpinController(
                cylinder=cylinder,
                ball=ball,
    ...
    def get_done(
        ...
        done_mask = self._step_counts[env_ids] >= 80
...
class _BallSpinController:
    """rotating_cylinder  ball_on_cylinder"""
    ...
    def __init__(
        ...
        if ball is not None:
            ball_translation, ball_orientation = ball.get_local_pose()
            if isinstance(ball_translation, torch.Tensor):
                offset = ball_translation.detach().clone().to(dtype=torch.float32)
            else:
                offset = torch.tensor(ball_translation, dtype=torch.float32)
            self.ball_offset = offset - self.base_translation

            if isinstance(ball_orientation, torch.Tensor):
                base_ori = ball_orientation.detach().clone().to(dtype=torch.float32)
            else:
                base_ori = torch.tensor(ball_orientation, dtype=torch.float32)
            self.ball_base_orientation = base_ori
        else:
            self.ball_offset = None
            self.ball_base_orientation = None
    ...
    def apply_current_pose(self):
        ...
        if (
            self.ball is None
            or self.ball_offset is None
            or self.ball_base_orientation is None
        ):
            return
        ...
        new_ball_orientation = self._quat_mul(rotation_quat, self.ball_base_orientation)
        self.ball.set_local_pose(
            translation=new_translation, orientation=new_ball_orientation
        )
        self.ball.set_linear_velocity(torch.zeros(3))
        self.ball.set_angular_velocity(torch.zeros(3))
