...
class RollInBoxEnv(TaskCameraBaseEnv):
    """
    Roll In Box environment for automatic data collection.
    This environment is designed for collecting data from phys4d scenes.
    get_done rolling_ball_1
    """
    ...
    def reset_idx(
        ...
        """Reset specified environments and clear their previous ball positions."""
    ...
    def get_done(
        ...
        """
        Check if rolling_ball_1 has come to rest by comparing position changes between steps.
        """
        if env_ids is None:
            env_ids = torch.arange(
                self.scene.num_envs, device=self.device, dtype=torch.long
        ...
        for env_id in env_ids:
            ...
            if (
                ...
                "rolling_ball"
                in self.scene.scene_manager._deformable_objects[env_id_int]
                and len(
                    self.scene.scene_manager._deformable_objects[env_id_int][
                        "rolling_ball"
                    ]
                )
                > 0
            ):
                ball = self.scene.scene_manager._deformable_objects[env_id_int][
                    "rolling_ball"
                ][0]
                ball_found = True
            ...
            if not ball_found:
                ...
                if debug and env_id_int == 0:
                    print(
                        "  env_0: Rolling ball not found in rigid or deformable objects"
                    )
                continue
            ...
            try:
                ...
                if "root_pose" not in ball_state:
                    is_done[env_id_int] = False
                    if debug and env_id_int == 0:
                        print("  env_0: root_pose not in state for rolling_ball")
                    continue
                ...
                if torch.isnan(current_pos).any():
                    is_done[env_id_int] = False
                    if debug and env_id_int == 0:
                        print("  env_0: rolling_ball root_pose contains nan")
                    continue
                ...
                if debug and env_id_int == 0:
                    ...
                    if env_id_int in self._prev_ball_positions:
                        ...
                        print(
                            f"Step: env_0 rolling_ball pos={pos_str}, pos_change={pos_change:.6f}, threshold={position_threshold} (from get_state)"
                        )
                    else:
                        print(
                            f"Step: env_0 rolling_ball pos={pos_str} (first step, no previous pos, from get_state)"
                        )
                ...
                if env_id_int in self._prev_ball_positions:
                    ...
                    if debug and env_id_int == 0:
                        print(
                            f"  env_0: rolling_ball is_done={is_done[env_id_int].item()}, pos_change={pos_change.item():.6f}"
                        )
                else:
                    is_done[env_id_int] = False
                ...
                is_done[env_id_int] = False
                if debug and env_id_int == 0:
                    print(f"  env_0: Error getting rolling_ball position: {e}")
                    import traceback
