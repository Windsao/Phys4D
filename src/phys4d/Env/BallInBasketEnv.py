...
class BallInBasketEnv(TaskCameraBaseEnv):
    """
    phys4d: Ball In Basket scene.
    get_done
    """
    ...
    def get_done(
        ...
        for env_id in env_ids:
            ...
            if (
                "ball" in self.scene.scene_manager._deformable_objects[env_id_int]
                and len(
                    self.scene.scene_manager._deformable_objects[env_id_int]["ball"]
                )
                > 0
            ):
                ball = self.scene.scene_manager._deformable_objects[env_id_int]["ball"][
                    0
                ]
