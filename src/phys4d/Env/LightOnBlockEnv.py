...
class LightOnBlockEnv(TaskCameraBaseEnv):
    """
    phys4d: Light On Block  MagicSim

    -  phys4d  TestLightOnBlockEnv  BlockDominoEnv
       rotating_cylinder  cube_on_cylinder  Z
    ...
    def __init__(self, config, cli_args, logger):
        ...
        self._cylinder_controllers: Dict[int, "_CylinderSpinController"] = {}
    ...
    def _init_cylinder_controllers(self):
        """env rotating_cylinder + cube_on_cylinder"""
        angular_speed = 4.0
        ...
        for env_id in range(self.num_envs):
            ...
            self._cylinder_controllers[env_id] = _CylinderSpinController(
                cylinder=cylinder,
                cube=cube,
...
class _CylinderSpinController:
    """rotating_cylinder  cube_on_cylinder"""
    ...
    def __init__(
        ...
        if cube is not None:
            cube_translation, cube_orientation = cube.get_local_pose()
            self.cube_offset = (
                torch.tensor(cube_translation, dtype=torch.float32)
                - self.base_translation
            )
            self.cube_base_orientation = torch.tensor(
                cube_orientation, dtype=torch.float32
            )
        else:
            self.cube_offset = None
            self.cube_base_orientation = None
    ...
    def apply_current_pose(self):
        ...
        if self.cube is not None and self.cube_offset is not None:
            angle_rad = self.initial_angle + self.angle
            cos_a = math.cos(angle_rad)
            sin_a = math.sin(angle_rad)
            rotated_offset = torch.tensor(
                [
                    self.cube_offset[0] * cos_a - self.cube_offset[1] * sin_a,
                    self.cube_offset[0] * sin_a + self.cube_offset[1] * cos_a,
                    self.cube_offset[2],
                ],
                dtype=torch.float32,
            )
            cube_new_translation = self.base_translation + rotated_offset

            if self.cube_base_orientation is not None:
                w2, x2, y2, z2 = self.cube_base_orientation
                ...
                cube_orientation = new_orientation

            self.cube.set_local_pose(
                translation=cube_new_translation, orientation=cube_orientation
            )
            self.cube.set_linear_velocity(torch.zeros(3))
            self.cube.set_angular_velocity(torch.zeros(3))
