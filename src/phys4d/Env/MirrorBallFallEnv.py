...
class MirrorBallFallEnv(TaskCameraBaseEnv):
    """
    Mirror Ball Fall environment for automatic data collection.
    ...
    def get_done(
        ...
        """
        Check if ball_1 has come to rest by comparing position changes between steps.
        ...
        Returns:
            Tensor of shape (num_envs,) with True for environments where ball is rested, False otherwise.
        """
        if env_ids is None:
            env_ids = torch.arange(
                self.scene.num_envs, device=self.device, dtype=torch.long
        ...
        for env_id in env_ids:
            ...
            if (
                "ball" in self.scene.scene_manager._rigid_objects[env_id_int]
                and len(self.scene.scene_manager._rigid_objects[env_id_int]["ball"]) > 0
            ):
                ball = self.scene.scene_manager._rigid_objects[env_id_int]["ball"][0]
                ...
                is_done[env_id_int] = False
                if debug and env_id_int == 0:
                    print("  env_0: Ball not found in deformable_objects")
