...
class BallHitsNothingEnv(TaskCameraBaseEnv):
    """
    phys4d: Ball Hits Nothing scene.
    get_done
    """
    ...
    def get_done(
        ...
        """
        Check if environments have reached 50 sim.step count.
         sim.step  +1
         env  50  True  env
        ...
        Returns:
            Tensor of shape (num_envs,) with True for environments where step_count >= 50, False otherwise.
        """
        if env_ids is None:
            env_ids = torch.arange(
                self.scene.num_envs, device=self.device, dtype=torch.long
        ...
        done_mask = self._step_counts[env_ids] >= 60
