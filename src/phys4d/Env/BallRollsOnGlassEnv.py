...
class BallRollsOnGlassEnv(TaskCameraBaseEnv):
    """
    phys4d: Ball Rolls On Glass scene.
    get_done  env  80  step
    """
    ...
    def get_done(
        ...
        """
        Check if environments have reached 80 sim.step count.
         sim.step  +1
         env  80  True  env

        Args:
            env_ids: Environment IDs to check. If None, checks all environments.
            position_threshold: Not used (kept for compatibility).
            debug: If True, print debug information for env_0.

        Returns:
            Tensor of shape (num_envs,) with True for environments where step_count >= 80, False otherwise.
        """
        if env_ids is None:
            env_ids = torch.arange(
                self.scene.num_envs, device=self.device, dtype=torch.long
