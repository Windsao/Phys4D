from typing import Any, Dict, Sequence
import math

import torch
import gymnasium as gym
from magicsim.StardardEnv.Camera.TaskCameraBaseEnv import TaskCameraBaseEnv


class BlockDominoEnv(TaskCameraBaseEnv):
    """
    phys4d: Block Domino scene.
    get_done  domino_blocks_6  z < 0.1
    """

    def __init__(self, config, cli_args, logger):
        super().__init__(config, cli_args, logger)
        self._primary_camera_name: str | None = None

        self._prev_ball_positions: Dict[int, torch.Tensor] = {}

        self._stick_controllers: Dict[int, "_StickSpinController"] = {}

        self._stick_controller_config = config

        self._step_counts: torch.Tensor | None = None

    def _quat_mul(self, q1: torch.Tensor, q2: torch.Tensor) -> torch.Tensor:
        """Multiply two quaternions."""
        w1, x1, y1, z1 = q1
        w2, x2, y2, z2 = q2
        return torch.tensor(
            [
                w1 * w2 - x1 * x2 - y1 * y2 - z1 * z2,
                w1 * x2 + x1 * w2 + y1 * z2 - z1 * y2,
                w1 * y2 - x1 * z2 + y1 * w2 + z1 * x2,
                w1 * z2 + x1 * y2 - y1 * x2 + z1 * w2,
            ],
            dtype=torch.float32,
            device=self.device,
        )

    def _yaw_to_quat(self, angle: float) -> torch.Tensor:
        """Convert yaw angle to quaternion."""
        half = angle * 0.5
        return torch.tensor(
            [math.cos(half), 0.0, 0.0, math.sin(half)],
            dtype=torch.float32,
            device=self.device,
        )

    def _init_stick_controllers(self):
        """Initialize stick spin controllers for all environments."""
        angular_speed = 4.0

        for env_id in range(self.num_envs):
            if (
                "stick" in self.scene.scene_manager._rigid_objects[env_id]
                and len(self.scene.scene_manager._rigid_objects[env_id]["stick"]) > 0
            ):
                stick = self.scene.scene_manager._rigid_objects[env_id]["stick"][0]

                pivot = None
                initial_angle_deg = 0.0
                config_angular_speed = angular_speed

                try:
                    scene_config = self.config.get("Scene", {})
                    objects_config = scene_config.get("objects", {})
                    stick_config = objects_config.get("stick", {})
                    stick_1_config = stick_config.get("stick_1", {})

                    if "pos" in stick_1_config:
                        pivot = torch.tensor(
                            [float(v) for v in stick_1_config["pos"]],
                            dtype=torch.float32,
                        )

                    controller_config = stick_1_config.get("controller", {})
                    if "initial_angle_deg" in controller_config:
                        initial_angle_deg = float(
                            controller_config["initial_angle_deg"]
                        )
                    if "angular_speed" in controller_config:
                        config_angular_speed = float(controller_config["angular_speed"])
                except Exception:
                    translation, _ = stick.get_local_pose()

                    pivot = torch.tensor(translation, dtype=torch.float32)

                self._stick_controllers[env_id] = _StickSpinController(
                    stick=stick,
                    angular_speed=config_angular_speed,
                    pivot_translation=pivot,
                    initial_angle_deg=initial_angle_deg,
                    device=self.device,
                    quat_mul_func=self._quat_mul,
                    yaw_to_quat_func=self._yaw_to_quat,
                )

    def step(
        self,
        camera_action: Dict[str, torch.Tensor] | None = None,
        env_ids: Sequence[int] | None = None,
        failed_env_ids: Sequence[int] | None = None,
    ):
        """Step the environment and update stick controllers."""

        if env_ids is None:
            env_ids = torch.arange(self.num_envs, device=self.device)
        elif not isinstance(env_ids, torch.Tensor):
            env_ids = torch.tensor(env_ids, device=self.device)

        physics_dt = float(getattr(self.scene.sim, "physics_dt", 1.0 / 60.0))

        for env_id in env_ids:
            env_id_int = int(env_id)
            if env_id_int in self._stick_controllers:
                controller = self._stick_controllers[env_id_int]
                controller.step(physics_dt)

        return super().step(
            camera_action=camera_action, env_ids=env_ids, failed_env_ids=failed_env_ids
        )

    def get_obs_space(self) -> gym.spaces.Dict:
        return gym.spaces.Dict({})

    def get_policy_obs(self, env_ids: Sequence[int] | None = None) -> Dict[str, Any]:
        if env_ids is None:
            env_ids = torch.arange(self.scene.num_envs, device=self.device)
        camera_info = self.scene.capture_manager.step(env_ids=env_ids)
        return {
            "camera_info": camera_info,
        }

    def get_privilege_obs(self, env_ids: Sequence[int] | None = None) -> Dict[str, Any]:
        return {}

    def process_camera_action(
        self,
        camera_action: Any | None,
        env_ids: Sequence[int] | None = None,
    ) -> Dict[str, Any] | None:
        return camera_action

    def _get_primary_camera_name(self) -> str:
        if self._primary_camera_name is None:
            camera_names = list(self.scene.camera_manager.camera_config.keys())
            if not camera_names:
                raise RuntimeError("No camera configured in camera_manager.")
            self._primary_camera_name = camera_names[0]
        return self._primary_camera_name

    def get_info(self, env_ids: Sequence[int] | None = None) -> Dict[str, Any]:
        """Return per-environment physics state for recording.

        The returned ``state`` is a list with one entry per env, where each entry
        is a nested dict converted to pure Python types (no torch.Tensors), so it
        is compatible with TaskCameraBaseEnv._check_dict_values_length.
        """

        def _to_python(obj: Any) -> Any:
            """Recursively convert tensors to Python / list types."""
            if isinstance(obj, torch.Tensor):
                if obj.ndim == 0:
                    return obj.item()
                return obj.detach().cpu().tolist()
            if isinstance(obj, dict):
                return {k: _to_python(v) for k, v in obj.items()}
            if isinstance(obj, (list, tuple)):
                return [_to_python(v) for v in obj]
            return obj

        if env_ids is None:
            env_ids = torch.arange(
                self.scene.num_envs, device=self.device, dtype=torch.long
            )
        elif not isinstance(env_ids, torch.Tensor):
            env_ids = torch.tensor(env_ids, device=self.device, dtype=torch.long)

        all_env_states = []
        for env_id in env_ids:
            env_id_int = int(env_id)
            state = self.scene.scene_manager.get_state(
                is_relative=True,
                env_ids=[env_id_int],
            )
            all_env_states.append(_to_python(state))

        return {"state": all_env_states}

    def get_reward(
        self,
        camera_action: Dict[str, torch.Tensor] | None,
        env_ids: Sequence[int] | None = None,
    ) -> torch.Tensor:
        return torch.zeros(self.num_envs, device=self.device)

    def get_termination(
        self,
        env_ids: Sequence[int] | None = None,
    ) -> tuple[torch.Tensor, torch.Tensor]:
        if env_ids is None:
            env_ids = torch.arange(
                self.scene.num_envs, device=self.device, dtype=torch.long
            )
        termination = torch.zeros(self.num_envs, dtype=torch.bool, device=self.device)
        truncated = torch.zeros_like(termination)
        return termination, truncated

    def reset(self, seed: int | None = None, options: dict[str, Any] | None = None):
        """Reset all environments and clear previous block positions."""
        obs, info = super().reset(seed=seed, options=options)

        self._prev_ball_positions.clear()

        if self._step_counts is None or self._step_counts.numel() != self.num_envs:
            self._step_counts = torch.zeros(
                self.num_envs, dtype=torch.long, device=self.device
            )
        else:
            self._step_counts.zero_()

        if not self._stick_controllers:
            self._init_stick_controllers()

        for controller in self._stick_controllers.values():
            controller.angle = 0.0
            controller._apply_current_pose()
        return obs, info

    def reset_idx(
        self,
        env_ids: Sequence[int] = None,
        seed: int | None = None,
        options: dict[str, Any] | None = None,
    ):
        """Reset specific environments and clear their previous block positions."""
        obs, info = super().reset_idx(env_ids=env_ids, seed=seed, options=options)

        if env_ids is not None:
            if not isinstance(env_ids, torch.Tensor):
                env_ids = torch.tensor(env_ids, device=self.device, dtype=torch.long)
            for env_id in env_ids:
                env_id_int = int(env_id)
                if env_id_int in self._prev_ball_positions:
                    del self._prev_ball_positions[env_id_int]

                if env_id_int in self._stick_controllers:
                    self._stick_controllers[env_id_int].angle = 0.0
                    self._stick_controllers[env_id_int]._apply_current_pose()
        else:
            self._prev_ball_positions.clear()

            if self._step_counts is None or self._step_counts.numel() != self.num_envs:
                self._step_counts = torch.zeros(
                    self.num_envs, dtype=torch.long, device=self.device
                )
            else:
                self._step_counts.zero_()

            for controller in self._stick_controllers.values():
                controller.angle = 0.0
                controller._apply_current_pose()
        return obs, info

    def get_done(
        self,
        env_ids: Sequence[int] | None = None,
        position_threshold: float = 0.001,
        debug: bool = False,
    ) -> torch.Tensor:
        """
        get_done sim.step  60

        Args:
            env_ids: Environment IDs to check. If None, checks all environments.
            position_threshold: Unused (kept for API compatibility).
            debug: If True, print debug information for env_0.

        Returns:
            Tensor of shape (num_envs,) with True for environments whose step
            count has reached 60, False otherwise.
        """
        if env_ids is None:
            env_ids = torch.arange(
                self.scene.num_envs, device=self.device, dtype=torch.long
            )
        elif not isinstance(env_ids, torch.Tensor):
            env_ids = torch.tensor(env_ids, device=self.device, dtype=torch.long)

        if self._step_counts is None or self._step_counts.numel() != self.num_envs:
            self._step_counts = torch.zeros(
                self.num_envs, dtype=torch.long, device=self.device
            )

        self._step_counts[env_ids] += 1

        is_done = torch.zeros(self.num_envs, dtype=torch.bool, device=self.device)
        done_mask = self._step_counts[env_ids] >= 100
        is_done[env_ids] = done_mask

        if done_mask.any():
            done_env_ids = env_ids[done_mask]
            self._step_counts[done_env_ids] = 0

        if debug:
            for env_id in env_ids:
                env_id_int = int(env_id)
                if env_id_int == 0:
                    print(
                        f"  env_0: step_count={self._step_counts[env_id_int].item()}, "
                        f"is_done={is_done[env_id_int].item()}"
                    )

        return is_done


class _StickSpinController:
    """Controller for spinning stick in block_domino scene."""

    def __init__(
        self,
        stick,
        angular_speed: float = 8.0,
        pivot_translation: torch.Tensor | None = None,
        initial_angle_deg: float = 0.0,
        device: torch.device = None,
        quat_mul_func=None,
        yaw_to_quat_func=None,
    ):
        self.stick = stick
        self.angular_speed = angular_speed
        self.angle = 0.0
        self.device = device or torch.device(
            "cuda" if torch.cuda.is_available() else "cpu"
        )
        self.quat_mul = quat_mul_func
        self.yaw_to_quat = yaw_to_quat_func

        translation, orientation = stick.get_local_pose()

        self.base_translation = (
            torch.tensor(pivot_translation, dtype=torch.float32)
            if pivot_translation is not None
            else torch.tensor(translation, dtype=torch.float32)
        )
        self.base_orientation = torch.tensor(orientation, dtype=torch.float32)
        self.initial_angle = math.radians(initial_angle_deg)
        self._apply_current_pose()

    def step(self, dt: float):
        """Update stick rotation."""
        if self.stick is None:
            return
        self.angle += self.angular_speed * dt
        self._apply_current_pose()

    def _apply_current_pose(self):
        """Apply current rotation to stick."""
        rotation_quat = self.yaw_to_quat(self.initial_angle + self.angle)
        new_orientation = self.quat_mul(rotation_quat, self.base_orientation)

        self.stick.set_local_pose(
            translation=self.base_translation, orientation=new_orientation
        )

        self.stick.set_linear_velocity(torch.zeros(3))
        self.stick.set_angular_velocity(torch.zeros(3))
